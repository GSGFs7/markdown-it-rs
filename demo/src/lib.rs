...
pub fn main() {
    ...
    let do_render = Closure::<dyn FnMut()>::new(move || {
        ...
        let preview = document
            .get_element_by_id("result-preview")
            .expect("no #result-preview");
        preview.set_inner_html(&html);
        let result_html = document
            .get_element_by_id("result-html")
            .expect("no #result-html");
        ...
        let result_ast = document
            .get_element_by_id("result-ast")
            .expect("no #result-ast");
    ...
    timeout.set(
        window
            .set_timeout_with_callback(do_render.as_ref().unchecked_ref())
            .ok(),
    );
    ...
    let tab_click = Closure::<dyn FnMut(_)>::new(move |event: web_sys::Event| {
        ...
        let document = window.document().unwrap();
        let tabs = document
            .query_selector_all("#tabs-toggle a.nav-link.active")
            .unwrap();
        ...
        let toggle = el
            .dataset()
            .get("toggle")
            .expect("no data-toggle attribute on anchor");
        let el = document
            .get_element_by_id(&toggle)
            .unwrap()
            .dyn_into::<web_sys::HtmlElement>()
            .unwrap();
    ...
    let tabs = document
        .query_selector_all("#tabs-toggle a.nav-link")
        .unwrap();
    for idx in 0..tabs.length() {
        tabs.get(idx)
            .unwrap()
